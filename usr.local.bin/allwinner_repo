...
##
REPO_URL='ssh://JingLun@61.143.53.198/git_repo/repo.git'
REPO_REV='master'
...
def _Clone(url, local, quiet):
  ...
  """
  loc=local.replace('.repo/repo','.repo')
  try:
    os.mkdir(loc)
  except OSError, e:
    print >>sys.stderr, 'creat :%s' % loc
          
    #raise CloneFailure()
  print >>sys.stderr, "git clone repo from % s" % REPO_URL 
  cmd = [GIT, 'clone', REPO_URL]
  try:
    proc = subprocess.Popen(cmd, cwd = loc)
  except OSError, e:
    print >>sys.stderr
